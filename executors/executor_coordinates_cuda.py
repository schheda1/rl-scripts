...
class ExecutorcoordinatesCuda(Executor):
    ...
    def get_exec_time_from_output(self, output):
        total_time = 0
        for line in output.splitlines():
            if "Average execution time of device transform" in line:
    ...
    def pre_measurement(self):
        source_dir = "../HeCBench/coordinates-cuda/"
        self.exec(["cp", "-R", source_dir, self.build_dir], quit_at_error=True)
        os.chdir(self.build_dir + "coordinates-cuda/")


if __name__ == "__main__":
    executor = ExecutorcoordinatesCuda("coordinates-cuda", include_files=["main.cu"])
    executor.execute()
